...
public final class PersonWarehouse implements Iterable<Person> {
    ...
    public Stream<Person> getUniquelyNamedPeople() {
        ...
        return people.stream().filter(person -> set.add(person.getName()));
    }
    ...
    public Stream<Person> getUniquelyNamedPeopleStartingWith(Character character) {
        return getUniquelyNamedPeople().filter( person -> (person.toString().charAt(0) == character));
    }
    ...
    public Stream<String> getAllAliases() {
        return getNestedAliases().flatMap( alias -> alias);
    }
